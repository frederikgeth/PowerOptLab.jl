...
algebraic=AlgebraicFormulation(.05/(1/2))
open(path,"w") do io
    println(io,"z,exact,soft,c2,soft_error,c2_error,soft_curvature,c2_curvature,algebraic,algebraic_error,algebraic_curvature")
    for z in range(-.8,.8;length=801)
        ...
        println(io,join((z,exact,s,c,s-exact,c-exact,
            ...
            hinge_value(z,algebraic),hinge_value(z,algebraic)-exact,
            hinge_derivatives(z,algebraic)[2]),","))
    end
end

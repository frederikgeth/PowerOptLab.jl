...
function _pwl_operator(m, f, r, si, so)
    ...
    key = (f.breakpoints, f.values, typeof(r), r.width, si, so)
...
function _formulate_pwl!(m,ctx,f,x,r,input_scale,output_scale)
    ...
    if all(==(first(f.values)),f.values)
        y = first(f.values)/so
    elseif r isa Union{SoftplusFormulation,LocalC2Formulation}
        expression = if ctx !== nothing && r isa SoftplusFormulation
            BMOPFTools.opf_piecewise_linear_expression(ctx,x,
                collect(f.breakpoints)./si,collect(f.values)./so; epsilon=r.width/si)
        else
            _pwl_operator(m,f,r,si,so)(x)
        end
        y = @variable(m, base_name="pwl_output")
        @constraint(m, y == expression)
    elseif r isa ComplementarityGraph
        expression = JuMP.AffExpr(first(f.values)/so)
        for (c,k) in _pwl_hinges(f)
            ...
            if k == first(f.breakpoints)
                JuMP.add_to_expression!(expression, c*si/so*x-c*k/so)
            elseif k != last(f.breakpoints)
                ...
                @constraint(m, positive-negative == x-k/si)
                @constraint(m, [positive,negative] in JuMP.MOI.Complements(2))
                push!(pairs,(positive,negative))
                JuMP.add_to_expression!(expression,c*si/so,positive)
            end
        end
        y = @variable(m, base_name="pwl_output")
        @constraint(m, y == expression)
    ...
    return PWLFormulationHandle(m,f,r,x,y,si,so,pairs)
end
...
function audit_pwl(h::PWLFormulationHandle)
    ...
    smooth = h.formulation isa Union{SoftplusFormulation,LocalC2Formulation}
    surrogate_error = smooth ? y-_pwl_smooth(h.curve,x,h.formulation) : nothing
    values = [(JuMP.value(a)*h.input_scale,JuMP.value(b)*h.input_scale) for (a,b) in h.pairs]

...
`strategy`, `semantics`, `reason`, affine `lines`, and remaining nonlinear smooth
...
"""
    plan_pwl_relation(curve, domain; relation=:equal, formulation=:auto, specialize=true)
...
function plan_pwl_relation(f::PWLFunction,domain;relation::Symbol=:equal,
        formulation=:auto,specialize::Bool=true)
    ...
    return PWLRelationPlan(f,bounds,relation,r,shape,strategy,semantics,reason,lines,active)
end
...
"""
    formulate_pwl_relation!(target, curve, input, output; domain=nothing,
        relation=:equal, formulation=:auto, specialize=true,
...
by both variables, effective domain, relation, formulation, specialization and
...
function formulate_pwl_relation!(target,f::PWLFunction,x,y;domain=nothing,
        relation::Symbol=:equal,formulation=:auto,specialize::Bool=true,
    ...
    build = function()
        ...
        plan = get!(plans,(f.breakpoints,f.values,f.input_unit,f.output_unit,
                bounds,relation,formulation,specialize)) do
            plan_pwl_relation(f,bounds;relation,formulation,specialize)
        end
        ...
        if plan.strategy in (:affine,:supporting_lines)
            for (a,b) in plan.lines
                push!(rows,_relation_row!(m,y,(a*si/so)*x+b/so,relation))
            end
        elseif plan.strategy == :local_c2
            ...
            push!(rows,_relation_row!(m,y,expression,relation))
    ...
    get!(build,cache,(target,f.breakpoints,f.values,f.input_unit,f.output_unit,
        bounds,x,y,relation,formulation,specialize,si,so))
end
...
function audit_pwl_relation(h::PWLRelationHandle)
    ...
    surrogate = r isa AbstractPWLSmoothing ? violation(y-_pwl_smooth(h.plan.curve,x,r)) : nothing
    lo,hi = h.plan.domain
